...
export function initCreator({
  ...
  launchRoom.onclick=()=>{try{const game=run(text.value);if(!game)return;const snapshot=multiplayerSnapshot();if(!snapshot.connected)throw new Error('Join or create a room first.');if(game.manifest.mode!=='multiplayer'&&!confirm('This file says mode: solo. Launch it for the room anyway?'))return;multiplayer?.launch?.({source:text.value,game});setStatus('Sending the game to the room…');dialog.close();}catch(error){setStatus('The room game could not start.','error',error.message);}};
