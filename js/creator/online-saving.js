...
export async function initOnlineSaving({ getSource, setCreatorStatus }) {
  ...
  const renderSession=(session)=>{const signedIn=Boolean(session?.user);authForm.hidden=signedIn;account.hidden=!signedIn;saveButton.hidden=!signedIn;accountLabel.textContent=signedIn?'Email connected':'';onlineStatus.textContent=signedIn?'Your online copies are private and not published.':'';};
